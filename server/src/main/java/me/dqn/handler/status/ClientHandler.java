...
/**
 * @author dqn
 * created at 2019/5/3 3:46
 */
public class ClientHandler implements HttpHandle {
    @Override
    public void handler(ChannelHandlerContext context, HttpResponseStatus status) {
        List<StateInfo> collect = ClientChannelManager.clientChannelSpeed.entrySet().stream().map(Map.Entry::getValue).collect(Collectors.toList());
        Gson gson = new Gson();
